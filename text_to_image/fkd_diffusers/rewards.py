...
def get_reward_function(reward_name, images, prompts, metric_to_chase="overall_score", device='cuda'):
    ...
    if reward_name == "ImageReward":
        return do_image_reward(images=images, prompts=prompts, device=device)
...
def do_human_preference_score(*, images, prompts, use_paths=False):
    if use_paths:
        scores = hpsv2.score(images, prompts)
        scores = [float(score) for score in scores]
    else:
        scores = []
        for i, image in enumerate(images):
            score = hpsv2.score(image, prompts[i])
...
# Compute ImageReward
def do_image_reward(*, images, prompts, device='cuda'):
    global REWARDS_DICT
    if REWARDS_DICT["ImageReward"] is None:
        REWARDS_DICT["ImageReward"] = rm_load("ImageReward-v1.0", device=device)
