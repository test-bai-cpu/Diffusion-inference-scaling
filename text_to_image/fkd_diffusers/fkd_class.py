...
class FKD:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.device = device
        self.resample_fn = resampling_function(
    ...
    def resample(
        ...
        if sampling_idx not in resampling_interval or self.lmbda == 0.0:
            return latents, None
        ...
        self.population_rs = self.population_rs.to(rs_candidates.device)
        ...
        if self.adaptive_resampling or sampling_idx == self.time_steps - 1:
            ...
            # Resample indices based on weights
            log_w = torch.log(w).unsqueeze(0)
            indices, _, log_w = self.resample_fn(log_w)
