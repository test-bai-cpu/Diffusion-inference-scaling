...
Maze-variant generator and scorer for OGBench pointmaze tasks.

For each task, up to K candidate routes are found on the base maze. An edit
BREAKS a route if any of its cells becomes a wall. The score grades by how
many routes break, and within that, by how hard the forced detour is (a
cheap local patch is mild; needing a totally different route is severe),
folded with how much demonstrated training traffic used the broken routes.
...
# ===========================================================================
# Traffic model + per-cell reroute severity
# ===========================================================================
...
def _reroute_severity(adj, cell):
    ...
    cell is removed. Disconnection -> large."""
...
if __name__ == "__main__":
    ...
    for task in ["task1", "task2", "task3", "task4", "task5"]:
        ...
        os.makedirs(f"maze_variants_seed{seed}", exist_ok=True)
        outpng = f"maze_variants_seed{seed}/{maze_type}_{task}.png"
        plot_suite(recs, model, outpng, maze_type, task)
        print(f"\nSaved plot to {outpng}")

        outjson = f"maze_variants_seed{seed}/{maze_type}_{task}.json"
