...
Loads an OGBench navigate dataset into a per-cell/per-edge ``TrafficModel``,
consumed by ``reroute_disruption.py``:

  * ``build_data_model``  -> reads ``TrafficModel``, builds the per-cell
                             reroute-severity ranking that biases WHERE edits land.
  * ``_route_traffic``    -> sums cell-visit traffic along a candidate route,
                             feeding the ``traffic_weighted_break`` term of RDI.
