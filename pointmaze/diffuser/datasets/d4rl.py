...
def load_environment(name, **env_kwargs):
    if type(name) != str:
        ## name is already an environment
        return name
    with suppress_output():
        # wrapped_env = gym.make(name)
        wrapped_env = ogbench.make_env_and_datasets(name, env_only=True, **env_kwargs)
...
def get_dataset(env):
    ...
    if 'pointmaze' in str(env.name).lower():
        env, dataset, val_dataset = ogbench.make_env_and_datasets(env.name)
        return dataset
